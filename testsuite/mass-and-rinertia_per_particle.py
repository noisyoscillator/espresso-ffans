...
class ThermoTest(ut.TestCase):
    ...
    def run_test_case(self, test_case):
        ...
        # A drift is starting since t >> t0 = mass / gamma
        t0_max = -1.0
        for k in range(2):
            t0_max = max(t0_max, max(mass / gamma_tr[k, :]), max(J[:] / gamma_rot_validate[k, :]))
        drift_steps_0 = int(math.floor(20 * t0_max / self.es.time_step))
        print("drift_steps_0 = {0}".format(drift_steps_0))
        ...
        if "EXTERNAL_FORCES" in espressomd.features():
            ...
            self.es.integrator.run(drift_steps_0)
            self.es.time = 0.0
            for k in range(2):
                self.es.part[k].pos = np.array([0.0, 0.0, 0.0])
            if "DIPOLES" in espressomd.features():
